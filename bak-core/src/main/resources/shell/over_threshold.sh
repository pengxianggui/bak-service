...
# 检查参数数量
if [ "$#" -ne 8 ]; then
    echo "Usage: $0 <mysql_ip> <mysql_port> <mysql_user> <database_name> <table_name> <time_field_name> <strategy> <strategy_value>"
    exit 1
fi
...
RESULT=$(mysql -h "$MYSQL_IP" -u "$MYSQL_USERNAME" -D "$DB_NAME" -se "$QUERY")
# 判断查询结果并返回布尔值
if [ "$RESULT" -eq 1 ]; then
    echo "true"
else
